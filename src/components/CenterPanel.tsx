...
import type { AppLanguage, ConceptTopographyReport, Link, Node } from '../types';
...
function buildConnectedComponentScopes(
  nodes: Node[],
  links: Link[],
  language: AppLanguage,
): TraditionScope[] {
  ...
  for (const node of nodes) {
    ...
    const dominant = dominantSource
      ? sourceLabel(dominantSource as Node['source'], language)
      : zh
        ? '混合'
        : 'Mixed';
    ...
    scopes.push({
      id: `scope-${root}`,
      label: `${dominant}${zh ? '傳統' : ' Tradition'} (${component.length} ${zh ? '節點' : 'nodes'})`,
...
function buildTaggedTraditionScopes(nodes: Node[], language: AppLanguage): TraditionScope[] {
  const zh = language === 'zh-Hant';
  ...
  const scopes: TraditionScope[] = Array.from(grouped.entries()).map(([id, entry]) => ({
    id: `tradition-${id}`,
    label: `${entry.label}${entry.independence ? ` (${entry.independence})` : ''} (${entry.nodeIds.size} ${zh ? '節點' : 'nodes'})`,
  ...
  if (unclassified.size > 0) {
    scopes.push({
      ...
      label: `${zh ? '未分類' : 'Unclassified'} (${unclassified.size} ${zh ? '節點' : 'nodes'})`,
...
function buildTraditionScopes(nodes: Node[], links: Link[], language: AppLanguage): TraditionScope[] {
  const tagged = buildTaggedTraditionScopes(nodes, language);
  if (tagged.length > 0) {
    return tagged;
  }
  return buildConnectedComponentScopes(nodes, links, language);
}
...
export function CenterPanel({
  ...
  const traditionScopes = useMemo(() => buildTraditionScopes(nodes, links, language), [language, links, nodes]);
  ...
  return (
    <section className="flex h-[56vh] w-full flex-col p-4 xl:h-screen xl:min-w-0 xl:flex-1 xl:p-5">
      <div className="mb-3 flex items-center justify-between">
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-[0.16em] text-olive">
            {zh ? '研究視窗' : 'Research Space'}
          </h2>
          <p className="mt-0.5 text-xs text-slate-500">
            {visibleNodes.length}/{nodes.length} {zh ? '節點' : 'nodes'} • {visibleLinks.length}/{links.length}{' '}
            {zh ? '連線' : 'links'}
          </p>
        </div>
        <div className="rounded-xl border border-amber-200 bg-white/70 p-1">
          ...
          >
            <span className="inline-flex items-center gap-1.5">
              <Network className="h-3.5 w-3.5" /> {zh ? '圖譜' : 'Graph'}
            </span>
          </button>
          <button
          ...
          >
            <span className="inline-flex items-center gap-1.5">
              <BarChart3 className="h-3.5 w-3.5" /> {zh ? '時間線' : 'Timeline'}
            </span>
          </button>
          <button
          ...
          >
            <span className="inline-flex items-center gap-1.5">
              <Box className="h-3.5 w-3.5" /> {zh ? '3D 地形圖' : '3D Map'}
            </span>
          </button>
        </div>
      </div>
      ...
      <div className="mb-2 flex flex-wrap items-center gap-2">
        {sourceLegend.map((item) => (
          ...
          >
            ...
            {sourceLabel(item.source, language)} ({sourceCounts[item.source] ?? 0})
          </span>
        ))}
      </div>
        ...
        {showLineControls ? (
          <div className="mt-2">
            <div className="mb-1 flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                ...
                >
                  ...
                  {showLinkLabels ? (zh ? '隱藏類型' : 'Hide Types') : zh ? '顯示類型' : 'Show Types'}
                </button>
                <button
            ...
            <div className="mb-1 flex flex-wrap items-center gap-1.5">
              ...
              >
                {zh ? '顯示所有連線類型' : 'Show All Line Types'}
              </button>
              {linkTypeCounts.map(([type, count]) => {
                ...
                return (
                  ...
                  >
                    {lineTypeLabel(type, language)} ({count})
                  </button>
                );
              })}
            </div>
            {linkTypeCounts.length === 0 ? (
              <span className="text-xs text-slate-500">
                {zh ? '目前範圍內沒有可用的連線類型。' : 'No link types in current scope.'}
              </span>
            ) : null}
          </div>
        ) : null}
        ...
        {loading ? (
          <div className="absolute inset-0 z-10 flex items-center justify-center rounded-2xl bg-white/65 backdrop-blur-[1px]">
            <p className="inline-flex items-center gap-2 rounded-xl border border-amber-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm">
              ...
              {zh ? '正在生成結構圖譜…' : 'Generating structural graph...'}
            </p>
          </div>
        ) : null}
        ...
        {!loading && visibleNodes.length === 0 ? (
          <div className="absolute inset-0 z-10 flex items-center justify-center rounded-2xl border border-dashed border-amber-300 bg-white/60">
            <p className="text-sm text-slate-600">
              {zh ? '請先執行研究追溯以生成圖譜。' : 'Run a research trace to generate the graph.'}
            </p>
          </div>
        ) : null}
      </div>
